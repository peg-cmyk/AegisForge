...
const AegisMuxer = (() => {
    ...
    class Engine {
        ...
        _initHdr() {
            try {
                if (this.fmt === "mp4" || this.fmt === "mov") {
                    ...
                    (this as any)._initAVI();
                } else if (this.fmt === "ogg") {
                    (this as any)._initOGG();
        ...
        _push(trk: Track, data: Uint8Array, isKey: boolean, pts: number, dts: number, dur: number, cto: number) {
            ...
            if (this.opt.mode === "fragmented") {
                ...
                (this as any)._flushOGG();
            } else if (this.fmt === "mp3") {
                (this as any)._flushMP3();
            } else {
        ...
        finalize() {
            ...
            try {
                if (this.fmt === "avi") {
                    (this as any)._finalizeAVI();
                } else if (this.fmt === "ogg") {
                    (this as any)._flushOGG(); (this as any)._writeOGGPage(new Uint8Array(0), true);
                } else if (this.fmt === "mp3") {
                    (this as any)._flushMP3();
    ...
    Engine.prototype._initOGG = function (this: any) {
        const at = this.at;
    ...
    Engine.prototype._writeOGGPage = function (this: any, data: Uint8Array, isEOS: boolean, isBOS?: boolean) {
    ...
    Engine.prototype._flushOGG = function (this: any) {
        const at = this.at;
        if (!at) return;
        while (at.queue.length) {
            ...
            (this as any)._writeOGGPage(f.d, false);
            f.d = null;
        }
    };


    Engine.prototype._flushMP3 = function (this: any) {
        const at = this.at;
        if (!at) return;
        while (at.queue.length) {
            const f = at.queue.shift();
